...
type ConnectivityHandler struct {
	...
	connectivityHandlerInitOnce sync.Once
}
...
func (s *ConnectivityHandler) OnHostMetadataV4V6Update(msg *proto.HostMetadataV4V6Update) (err error) {
	...
	if localNodeSpec.Name == *config.NodeName &&
		...
		s.connectivityHandlerInitOnce.Do(func() {
			// this is needed as connectivity does not support support
			// starting without knowing node IPs
			// TODO: we should properly implement the node address update
			err = s.connectivityHandlerInit()
			if err != nil {
				s.log.WithError(err).Errorf("Error in connectivityHandlerInit")
			}
		})
	}
	return nil
