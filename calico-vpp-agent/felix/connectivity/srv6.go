...
func isSRv6Enabled() bool {
	enabled := config.GetCalicoVppFeatureGates().SRv6Enabled
	return enabled != nil && *enabled
}

func NewSRv6Provider(vpp *vpplink.VppLink, clientv3 calicov3cli.Interface, cache *cache.Cache, log *logrus.Entry) *SRv6Provider {
	p := &SRv6Provider{
		vpp:            vpp,
		log:            log,
		cache:          cache,
...
func (p *SRv6Provider) setEncapSource() (err error) {
	...
	if p.cache.GetNodeIP6() == nil {
		return fmt.Errorf("no ip6 found for node")
	}
	if err = p.vpp.SetEncapSource(*p.cache.GetNodeIP6()); err != nil {
		p.log.Errorf("SRv6Provider setEncapSource: %v", err)
		return errors.Wrapf(err, "SRv6Provider setEncapSource")
	}
	p.log.Debugf("SRv6Provider setEncapSource with IP6 %s", p.cache.GetNodeIP6().String())
	return err
}
