	...
	Describe("Runtime config", func() {
		...
		Context("HostMetadata updates", func() {
			...
			It("should handle hostMetadataV4V6 updates of another node", func() {
				By("receiving a hostmetadatav4v6 update of another node")
				go func() {
					<-felixServer.GotOurNodeBGPchan
				}()
				nodeName := "host"
				...
				Expect(err).ToNot(HaveOccurred(),
					"failed to handle hostMetadataV4V6Update")
				expectCnatSnatContain(vpp, []string{"ip4: 1.1.1.1;0", "ip6: f::f;0"}, []string{"11.11.11.11", "f::d"})

				By("receiving a hostmetadatav4v6 remove of another node")
				err = felixServer.handleHostMetadataV4V6Remove(&proto.HostMetadataV4V6Remove{
					Hostname: "host2",
				}, false)
				Expect(err).ToNot(HaveOccurred(),
