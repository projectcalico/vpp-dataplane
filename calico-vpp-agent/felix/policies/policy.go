...
func (s *PoliciesHandler) OnActivePolicyUpdate(msg *proto.ActivePolicyUpdate) (err error) {
	state := s.GetState()
	id := fromProtoPolicyID(msg.Id, defaultNetwork)
	...
	for network := range s.cache.NetworkDefinitions {
		id := fromProtoPolicyID(msg.Id, network)
...
func (s *PoliciesHandler) OnActivePolicyRemove(msg *proto.ActivePolicyRemove) (err error) {
	...
	for policyID := range state.Policies {
		if policyID.Name == id.Name && policyID.Namespace == id.Namespace && policyID.Kind == id.Kind {
			existing, ok := state.Policies[policyID]
			if !ok {
				s.log.Warnf("Received policy delete for kind %s namespace %s name %s that doesn't exists", id.Kind, id.Namespace, id.Name)
				return nil
			}
			if !s.state.IsPending() {
