...
type BGPHandler struct {
	...
	bgpPeers               map[string]*common.LocalBGPPeer
	bgpConnectivityHandler common.BGPConnectivityHandler
}

// NewBGPHandler creates a new BGP handler instance
func NewBGPHandler(bgpServer *bgpserver.BgpServer, log *logrus.Entry) *BGPHandler {
	return &BGPHandler{
		log:        log,
		BGPServer:  bgpServer,
		bgpFilters: make(map[string]*calicov3.BGPFilter),
		bgpPeers:   make(map[string]*common.LocalBGPPeer),
	}
}
...
func (h *BGPHandler) InjectRoute(path *bgpapi.Path) error {
	...
	if path.IsWithdraw {
		if h.bgpConnectivityHandler != nil {
			return h.bgpConnectivityHandler.OnConnectivityDeleted(cn)
		}
	} else {
		if h.bgpConnectivityHandler != nil {
			return h.bgpConnectivityHandler.OnConnectivityAdded(cn)
		}
	}
	return nil
}
...
func (h *BGPHandler) InjectSRv6Policy(path *bgpapi.Path) error {
	...
	if path.IsWithdraw {
		if h.bgpConnectivityHandler != nil {
			return h.bgpConnectivityHandler.OnSRv6PolicyDeleted(cn)
		}
	} else {
		if h.bgpConnectivityHandler != nil {
			return h.bgpConnectivityHandler.OnSRv6PolicyAdded(cn)
		}
	}
	return nil
}
...
func (h *BGPHandler) NewBGPPolicyAndAssignment(name string, rulesv4 []calicov3.BGPFilterRuleV4, rulesv6 []calicov3.BGPFilterRuleV6, neighborName string, dir bgpapi.PolicyDirection) (*common.BGPPrefixesPolicyAndAssignment, error) {
	...
	return &common.BGPPrefixesPolicyAndAssignment{PolicyAssignment: PA, Policy: pol, Prefixes: prefixes}, nil
}

// filterPeer creates policies in gobgp representing bgpfilters for the peer
func (h *BGPHandler) filterPeer(peerAddress string, filterNames []string) (map[string]*common.ImpExpPol, error) {
	BGPPolicies := make(map[string]*common.ImpExpPol)
...
func (h *BGPHandler) createFilterPolicy(peerAddress string, filterName string, neighborSet string) (*common.ImpExpPol, error) {
	...
	for _, pol := range []*common.BGPPrefixesPolicyAndAssignment{imppol, exppol} {
	...
	return &common.ImpExpPol{Imp: imppol, Exp: exppol}, nil
}

// deleteFilterPolicy deletes policies and their assignments in gobgp
func (h *BGPHandler) deleteFilterPolicy(impExpPol *common.ImpExpPol) error {
	for _, pol := range []*common.BGPPrefixesPolicyAndAssignment{impExpPol.Imp, impExpPol.Exp} {
...
// SetBGPConnectivityHandler sets the BGP connectivity handler for route injection
func (h *BGPHandler) SetBGPConnectivityHandler(handler common.BGPConnectivityHandler) {
	h.bgpConnectivityHandler = handler
}

// HandleBGPPeerAdded handles BGP peer addition directly
func (h *BGPHandler) HandleBGPPeerAdded(localPeer *common.LocalBGPPeer) error {
...
func (h *BGPHandler) HandleBGPPeerUpdated(localPeer *common.LocalBGPPeer, oldPeer *common.LocalBGPPeer) error {
	h.log.Debugf("BGP handler processing BGP peer updated")
	peer := localPeer.Peer
	filters := localPeer.BGPFilterNames
	h.log.Infof("bgp(upd) neighbor=%s", peer.Conf.NeighborAddress)

	var BGPPolicies map[string]*common.ImpExpPol
	if !common.CompareStringSlices(localPeer.BGPFilterNames, oldPeer.BGPFilterNames) { // update filters
