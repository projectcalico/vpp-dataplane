...
func (s *Server) GetLocalService(service *v1.Service, ep []*discoveryv1.EndpointSlice) (localService *LocalService) {
	...
	for _, servicePort := range service.Spec.Ports {
		for _, cip := range clusterIPs {
			if !cip.IsUnspecified() && len(cip) > 0 {
				entry := s.buildCnatEntryForServicePort(&servicePort, service, ep, cip, false /* isNodePort */, *serviceSpec)
		...
		for _, eip := range service.Spec.ExternalIPs {
			...
			if !extIP.IsUnspecified() && len(extIP) > 0 {
				entry := s.buildCnatEntryForServicePort(&servicePort, service, ep, extIP, false /* isNodePort */, *serviceSpec)
				localService.Entries = append(localService.Entries, *entry)
		...
		for _, ingress := range service.Status.LoadBalancer.Ingress {
			...
			if !ingressIP.IsUnspecified() && len(ingressIP) > 0 {
				entry := s.buildCnatEntryForServicePort(&servicePort, service, ep, ingressIP, false /* isNodePort */, *serviceSpec)
				localService.Entries = append(localService.Entries, *entry)
				if IsLocalOnly(service) && len(entry.Backends) > 0 {
		...
		if service.Spec.Type == v1.ServiceTypeNodePort {
			for _, nip := range nodeIPs {
				if !nip.IsUnspecified() && len(nip) > 0 {
					entry := s.buildCnatEntryForServicePort(&servicePort, service, ep, nip, true /* isNodePort */, *serviceSpec)
					localService.Entries = append(localService.Entries, *entry)
				}
			}
		}
...
func (s *Server) deleteServiceByName(serviceID string) {
	...
	s.log.Debugf("deleting service %s from endpointSlicesByService map", serviceID)
	delete(s.endpointSlicesByService, serviceID)
