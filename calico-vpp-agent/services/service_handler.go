...
func buildCnatEntryForServicePort(servicePort *v1.ServicePort, service *v1.Service, epslices []*discoveryv1.EndpointSlice, serviceIP net.IP, isNodePort bool, svcInfo serviceInfo) *types.CnatTranslateEntry {
	...
	for _, servicePort := range service.Spec.Ports {
		for _, cip := range clusterIPs {
			if !cip.IsUnspecified() && len(cip) > 0 {
				entry := buildCnatEntryForServicePort(&servicePort, service, ep, cip, false /* isNodePort */, *serviceSpec)
		...
		if service.Spec.Type == v1.ServiceTypeNodePort {
			for _, nip := range nodeIPs {
				if !nip.IsUnspecified() && len(nip) > 0 {
					entry := buildCnatEntryForServicePort(&servicePort, service, ep, nip, true /* isNodePort */, *serviceSpec)
					localService.Entries = append(localService.Entries, *entry)
				}
			}
		}
