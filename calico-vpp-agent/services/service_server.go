...
type Server struct {
	...
	endpointSlicesByServiceLock sync.Mutex
...
func (s *Server) resolveLocalServiceFromService(service *v1.Service) *LocalService {
	...
	eps := s.findMatchingEndpoint(service)
	if len(eps) == 0 {
		s.log.Debugf("svc() no endpoints found for service=%s", serviceID(&service.ObjectMeta))
		return nil
	}
	s.endpointSlicesByServiceLock.Lock()
	_, found := s.endpointSlicesByService[serviceID(&service.ObjectMeta)]
	if !found {
		s.log.Debugf("adding service %s to endpointSlicesByService map", serviceID(&service.ObjectMeta))
		s.endpointSlicesByService[serviceID(&service.ObjectMeta)] = make(map[string]*discoveryv1.EndpointSlice)
	}
	s.endpointSlicesByServiceLock.Unlock()
	return s.GetLocalService(service, eps)
}

func (s *Server) resolveLocalServiceFromEndpointSlice(ep *discoveryv1.EndpointSlice, newSlice bool) *LocalService {
	if ep == nil {
		return nil
	}
	...
	s.endpointSlicesByServiceLock.Lock()
	defer s.endpointSlicesByServiceLock.Unlock()
	_, found := s.endpointSlicesByService[serviceID(&service.ObjectMeta)]
	if !found {
		s.log.Debugf("adding service %s to endpointSlicesByService map", serviceID(&service.ObjectMeta))
		s.endpointSlicesByService[serviceID(&service.ObjectMeta)] = make(map[string]*discoveryv1.EndpointSlice)
	}
	s.log.Debugf("adding endpoint %s to endpointSlicesByService map for service %s", ep.Name, serviceID(&service.ObjectMeta))
	s.endpointSlicesByService[serviceID(&service.ObjectMeta)][ep.Name] = ep

	if newSlice {
		// find all matching endpoints, we may have multiple endpointslices
		// for the same service, so we need to recheck them in the map cache,
		// otherwise they will get overridden by the current
		svceps := []*discoveryv1.EndpointSlice{}
		for _, svcep := range s.endpointSlicesByService[serviceID(&service.ObjectMeta)] {
			svceps = append(svceps, svcep)
		}
		return s.GetLocalService(service, svceps)
	} else {
		return s.GetLocalService(service, []*discoveryv1.EndpointSlice{ep})
	}
}
		...
		cache.InformerOptions{
			...
			Handler: cache.ResourceEventHandlerFuncs{
				AddFunc: func(obj interface{}) {
					...
					local := server.resolveLocalServiceFromEndpointSlice(slice, true /* newSlice */)
					server.handleServiceEndpointEvent(local, nil)
				},
				UpdateFunc: func(old, new interface{}) {
					oldSlice, ok := old.(*discoveryv1.EndpointSlice)
					if !ok {
						panic("wrong type for obj, not *discoveryv1.EndpointSlice")
					}
					newSlice, ok := new.(*discoveryv1.EndpointSlice)
					if !ok {
						panic("wrong type for obj, not *discoveryv1.EndpointSlice")
					}
					oldLocal := server.resolveLocalServiceFromEndpointSlice(oldSlice, false /* newSlice */)
					newLocal := server.resolveLocalServiceFromEndpointSlice(newSlice, true /* newSlice */)
					server.handleServiceEndpointEvent(newLocal, oldLocal)
				},
				DeleteFunc: func(obj interface{}) {
					slice, ok := obj.(*discoveryv1.EndpointSlice)
					if !ok {
						slice, ok = obj.(cache.DeletedFinalStateUnknown).Obj.(*discoveryv1.EndpointSlice)
						if !ok {
							panic(fmt.Sprintf("obj.(cache.DeletedFinalStateUnknown).Obj not a (*v1.EndpointSlice) %v", obj))
						}
					}
					server.deleteServiceByName(serviceID(&slice.ObjectMeta))
				},
			}},
	)
...
func (s *Server) findMatchingService(ep *discoveryv1.EndpointSlice) *v1.Service {
	...
	if strings.LastIndex(key, "-") > 0 {
		key = key[:strings.LastIndex(key, "-")] // with endpointslices the key has an additional hash
	}
...
func (s *Server) findMatchingEndpoint(service *v1.Service) []*discoveryv1.EndpointSlice {
	epslices := []*discoveryv1.EndpointSlice{}
	endpointslices := s.endpointStore.List()
	for _, ele := range endpointslices {
		eps, ok := ele.(*discoveryv1.EndpointSlice)
		if !ok {
			panic("wrong type for obj, not *discoveryv1.EndpointSlice")
		}
		labelSvcName, ok := eps.Labels["kubernetes.io/service-name"]
		if !ok {
			continue
		}
		if labelSvcName == service.Name && eps.Namespace == service.Namespace {
