	...
	for t.Alive() {
		...
		for {
			...
			case event, ok := <-w.watcher.ResultChan():
				...
				bgpConf, ok := event.Object.(*calicov3.BGPConfiguration)
				if !ok {
					w.log.Error("Unexpected object type in BGP configuration event")
					goto restart
				}
				w.currentWatchRevision = bgpConf.GetResourceVersion()
				switch event.Type {
				case watch.Error:
