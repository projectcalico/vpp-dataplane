...
// SetConnectivityHandler sets the connectivity handler for route injection
func (h *BGPHandler) SetConnectivityHandler(handler ConnectivityHandler) {
	h.connectivityHandler = handler
}
...
func (h *BGPHandler) HandleBGPPathDeleted(path *bgpapi.Path) error {
	...
	return h.deleteBGPPath(path)
}

// addBGPPath adds a BGP path
func (h *BGPHandler) addBGPPath(path *bgpapi.Path) error {
...
// deleteBGPPath deletes a BGP path
func (h *BGPHandler) deleteBGPPath(path *bgpapi.Path) error {
...
// injectRoute processes BGP route injection for connectivity
func (h *BGPHandler) InjectRoute(path *bgpapi.Path) error {
	h.log.Debugf("Injecting route: %s", path.Nlri)
	...
	if err := path.Nlri.UnmarshalTo(ipv4unicastNLRI); err == nil {
		...
		vpn = true
		h.log.Debugf("BGP VPN update: %+v", labeledVPNIPAddressPrefixNlri)
	...
	if path.IsWithdraw {
		if h.connectivityHandler != nil {
			return h.connectivityHandler.OnConnectivityDeleted(cn)
		}
	} else {
		if h.connectivityHandler != nil {
			return h.connectivityHandler.OnConnectivityAdded(cn)
		}
	}
	return nil
}

// InjectSRv6Policy processes SRv6 policy injection for connectivity
func (h *BGPHandler) InjectSRv6Policy(path *bgpapi.Path) error {
	...
	if path.IsWithdraw {
		if h.connectivityHandler != nil {
			return h.connectivityHandler.OnSRv6PolicyDeleted(cn)
		}
	} else {
		if h.connectivityHandler != nil {
			return h.connectivityHandler.OnSRv6PolicyAdded(cn)
		}
	}
	return nil
}
...
// getSRPolicy extracts SRv6 policy information from BGP path
