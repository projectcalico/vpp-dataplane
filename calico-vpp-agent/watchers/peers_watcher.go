	...
	for t.Alive() {
		...
		case event, ok := <-w.watcher.ResultChan():
			...
			case watch.EventType(api.WatchError):
				...
				goto restart
			case watch.Added:
				peer, ok := event.Object.(*calicov3.BGPPeer)
				if !ok {
					w.log.Errorf("Unexpected peer watch add object type: %T", event.Object)
					continue
				}
			...
			case watch.Modified:
				peer, ok := event.Object.(*calicov3.BGPPeer)
				if !ok {
					w.log.Errorf("Unexpected peer watch update object type: %T", event.Object)
					continue
				}
			...
			case watch.Deleted:
				peer, ok := event.Previous.(*calicov3.BGPPeer)
				if !ok {
					w.log.Errorf("Unexpected peer watch delete object type: %T", event.Previous)
					continue
				}
