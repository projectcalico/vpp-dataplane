...
func SwapDriver(log *logrus.Entry, pciDevice, newDriver string, addID bool) error {
	if pciDevice == "" {
...
// CreateVppLink creates a new link to VPP and waits for VPP to be up and
// running (by issuing a simple VPP API call). This is the canonical entry
// point used by both vpp-manager and calico-vpp-agent.
...
// CreateVppLinkInRetryLoop creates a new link to VPP and waits for VPP to be
// up and running, retrying in a loop up to `timeout` with `retry` interval
// between attempts. The simple GetVPPVersion call after Connect ensures VPP
// has finished its API initialization before we return the handle.
func CreateVppLinkInRetryLoop(socket string, log *logrus.Entry,
	timeout time.Duration, retry time.Duration) (vpp *vpplink.VppLink, err error) {
	...
	for i := 0; i < maxRetry; i++ {
		...
		// Verify the API is actually serving by issuing a trivial call.
		version, verr := vpp.GetVPPVersion()
		if verr != nil {
			log.Warnf("Try [%d/%d] broken vpplink: %v", i, maxRetry, verr)
			time.Sleep(retry)
			continue
		}
	...
	return nil, errors.Errorf("Cannot connect to VPP after %d tries", maxRetry)
}
...
func CleanupCoreFiles(log *logrus.Entry, corePattern string, maxCoreFiles int) error {
	if corePattern == "" {
		return nil
	}
...
func PingCalicoVpp(log *logrus.Entry) error {
	dat, err := os.ReadFile(CalicoVppPidFile)
	if err != nil {
...
func ClearVppManagerFiles() error {
	err := os.Remove(VppManagerInfoFile)
...
func HasAddr(ip net.IP, addresses []netlink.Addr) bool {
