...
func (h *NetworkManagerHook) tweakNetworkFile(interfaceName string) error {
	...
	return h.reloadAndRestartService("systemd-networkd")
}
...
func (h *NetworkManagerHook) beforeVppRun() error {
	...
	for _, intf := range h.vppManagerParams.InterfacesByID {
		err = h.saveNetworkFile(intf.Spec.InterfaceName)
		if err != nil {
			return err
		}
	}
...
func (h *NetworkManagerHook) vppRunning() error {
	...
	for _, intf := range h.vppManagerParams.InterfacesByID {
		err = h.tweakNetworkFile(intf.Spec.InterfaceName)
		if err != nil {
			return err
		}
	}
...
func (h *NetworkManagerHook) vppDoneOk() error {
	...
	for _, intf := range h.vppManagerParams.InterfacesByID {
		err = h.removeTweakedNetworkFile(intf.Spec.InterfaceName)
		if err != nil {
			return err
		}
	}
...
// 3. If native hooks disabled & no user script -> no-op
