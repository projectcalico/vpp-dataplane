...
func (d *DPDKDriver) IsSupported(warn bool) (supported bool) {
	var ret bool
	supported = true
	ret = d.intf.State.PciID != ""
	if !ret && warn {
		d.log.Warnf("did not find  pci device id for interface")
	}
...
func (d *DPDKDriver) getFinalDriver() string {
	if d.intf.State.DoSwapDriver {
		return d.intf.Spec.NewDriverName
	}
	return d.intf.State.Driver
}
...
func (d *DPDKDriver) UpdateVppConfigFile(template string) string {
	...
	if d.params.AvailableHugePages > 0 {
		template = fmt.Sprintf(
			...
			template, d.intf.State.PciID, d.intf.Spec.NumRxQueues, d.intf.Spec.NumTxQueues,
			d.intf.Spec.RxQueueSize, d.intf.Spec.TxQueueSize, "main-"+d.intf.Spec.InterfaceName,
		)
		...
		if buffersStanzaRegex.MatchString(template) {
			if !buffersNoHugeStanzaRegex.MatchString(template) {
				...
				d.log.Infof("Found buffers configuration in template")
			}
		} else {
...
func (d *DPDKDriver) restoreInterfaceName() error {
	newName, err := config.GetInterfaceNameFromPci(d.intf.State.PciID)
	if err != nil {
		return errors.Wrapf(err, "Error getting new if name for %s: %v", newName, d.intf.State.PciID)
	}
	if newName == d.intf.Spec.InterfaceName {
		return nil
	}
	...
	err = netlink.LinkSetName(link, d.intf.Spec.InterfaceName)
	if err != nil {
		return errors.Wrapf(err, "Error setting new if name for %s: %v", d.intf.Spec.InterfaceName, link)
	}
	return nil
}
...
func (d *DPDKDriver) RestoreLinux() {
	...
	for i := 0; i < 10; i++ {
		...
		if err != nil {
			d.log.Warnf("Error restoring if name %s", err)
		} else {
			break
		}
	...
	if d.getFinalDriver() == config.DriverVfioPci &&
		...
		if err != nil {
			d.log.Errorf("failed to configure vfio %s", err)
		}
	}
}
...
func (d *DPDKDriver) CreateMainVppInterface(vpp *vpplink.VppLink, vppPid int, uplinkSpec *config.UplinkInterfaceSpec) (err error) {
	...
	d.log.Debugf("Found interface with swIfIndex %d for %s", swIfIndex, d.intf.Spec.InterfaceName)
	d.intf.Spec.SwIfIndex = swIfIndex
	err = vpp.SetInterfaceMacAddress(swIfIndex, d.intf.State.HardwareAddr)
	if err != nil && gerrors.Is(err, types.VppErrorUnimplemented) {
		d.log.Warn("Setting dpdk interface mac address in vpp unsupported")
	} else if err != nil {
...
func NewDPDKDriver(params *vppmanagerparams.VppManagerParams,
	intf *vppmanagerparams.VppManagerInterface,
	log *logrus.Entry) *DPDKDriver {
	return &DPDKDriver{
		UplinkDriverData: UplinkDriverData{
			name:   NativeDriverDpdk,
			params: params,
			intf:   intf,
			log:    log,
		},
