...
const (
	...
	HookCaptureHostUdevProps HookPoint = "CAPTURE_HOST_UDEV_PROPS"
...
func (h *NetworkManagerHook) createUdevNetNameRules() error {
	...
	for interfaceName, props := range h.udevProps {
		if props == nil || props.MacAddress == "" {
			h.log.Warnf("NetworkManagerHook: Skipping %s - no MAC address captured", interfaceName)
			continue
		}
...
// removeUdevNetNameRule removes the udev rule file created for restoring ID_NET_NAME_* properties
func (h *NetworkManagerHook) removeUdevNetNameRule() error {
