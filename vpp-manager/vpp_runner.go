...
func (v *VppRunner) configureVpp(ifState *config.LinuxInterfaceState, ifSpec config.UplinkInterfaceSpec) (err error) {
	...
	err = v.vpp.SetInterfaceRxMode(ifSpec.SwIfIndex, types.AllQueues, config.GetRxMode(ifSpec.RxMode))
	if err != nil {
		log.Warnf("%v", err)
	}
	...
	err = v.vpp.SetInterfaceRxMode(tapSwIfIndex, types.AllQueues, config.GetRxMode(v.params.DefaultTap.RxMode))
	if err != nil {
